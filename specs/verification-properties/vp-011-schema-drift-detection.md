...
proptest! {
    ...
    fn drift_detects_any_change(
        ...
        // Two-tier severity: canonical changes → high, annotation changes → info
        if matches!(mutation, ToolMutation::ChangeAnnotation { .. }) {
            prop_assert!(findings.iter().all(|f| f.severity == Severity::Info));
        } else if matches!(mutation, ToolMutation::ChangeName { .. }
            | ToolMutation::ChangeDescription { .. }
            | ToolMutation::ChangeInputSchema { .. }) {
            prop_assert!(findings.iter().any(|f| f.severity >= Severity::High));
        }
    }
}
```
...
| Mutation Type | Drift Finding Expected | Severity Tier |
|--------------|----------------------|---------------|
| Name change | Yes — tool renamed | ≥ high (canonical) |
| Description change | Yes — description altered | ≥ high (canonical) |
| Schema change | Yes — input schema modified | ≥ high (canonical) |
| Annotation change | Yes — annotation modified | info (auxiliary) |
| Tool added | Yes — new tool appeared | info (new tool) |
| Tool removed | Yes — tool disappeared | medium (tool removed) |
